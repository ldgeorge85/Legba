...
**How it works (under the hood).** Legba ingests data from any source — pulled on a cadence, or pushed to a webhook — and fuses it into a provenance-tracked knowledge substrate: signals → facts / nexuses / situations (a temporal knowledge graph) → four bounded reasoning **units** → a per-country **composition** → a world **composition** → a banded **scorecard**, every output traceable to its sources via lineage and hash-chained receipt chains. It turns declarative descriptors — sources, targets, and analysts — into running actors over a shared substrate: a Dapr virtual-actor runtime reads each registered descriptor and stands up the corresponding actor, which acquires raw observations, routes them, reasons over them, verifies, and writes findings back. There is no code to write per feed, per target, or per analysis — you declare what to watch and how to reason, register it, and the runtime executes it. Derived rows carry full lineage plus a SHA-256 hash-chained receipt; the descriptor registry keeps a content-hashed, Ed25519-signed audit log of every change.

What makes Legba different is the discipline, not privileged data or cleverer analytics — plenty of tools have those. It's that every claim is cited, second-pass verified, and auditable through a hash-chained receipt chain to its source, on an engine you **run yourself (AGPL)** and **configure by declaration** — so you can *check* the intelligence, not just consume it.
