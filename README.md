...
- **Operator UI.** A Dockview single-page app (served by Caddy) surfacing findings, situations, predictions, the actor roster, lineage walks, the entity graph, budget ledgers, and inline descriptor editing — see [docs/UI.md](docs/UI.md). A unified **Live Feed** panel (`system.findings`) merges findings and signals from two NATS tails (`analyst.*.finding` + `legba.signals.>`) into one stream, with a Live on/off toggle, a Source filter (All / Findings / Signals), and near-duplicate clustering.
...
| Coalescing analysts → findings / predictions | **built** | Twelve built-in analyst kinds; reactive + cadence firing. |
