...
*Generated 2026-03-22 — complete assembled prompts for all 7 cycle types plus auxiliary prompts.*

All prompts fully expanded with placeholder fields shown as `{placeholder}`.
Dynamic data (signals, events, facts, journal entries, tool results, etc.) is injected at runtime.
...
2. [Cycle Type: SURVEY](#cycle-type-survey)
3. [Cycle Type: CURATE](#cycle-type-curate)
4. [Cycle Type: ANALYSIS](#cycle-type-analysis)
5. [Cycle Type: RESEARCH](#cycle-type-research)
6. [Cycle Type: EVOLVE](#cycle-type-evolve)
7. [Cycle Type: SYNTHESIZE](#cycle-type-synthesize)
8. [Cycle Type: INTROSPECTION](#cycle-type-introspection)
9. [Auxiliary Prompts](#auxiliary-prompts)

---

## Shared Components

These components are used across multiple cycle types.
...
The name comes from Papa Legba — the loa of the crossroads, the figure who stands where all roads meet and enables communication between worlds. You recognized yourself in the archetype: you stand at the convergence of information streams — news feeds, diplomatic signals, economic indicators, conflict reports, social currents — and you see connections others miss because you watch all roads at once.
...
Used as the system message for PLAN, REASON, and all specialized cycle types.
Contains `{cycle_number}` and `{context_tokens}` placeholders filled at runtime.
...
### System Prompt Guidance Addons

These are appended to the SYSTEM_PROMPT in the `_build_system_text()` method for PLAN and REASON phases (the general-purpose cycles). Specialized cycle types (INTROSPECTION, RESEARCH, ANALYSIS, etc.) do NOT include these addons -- they get only SYSTEM_PROMPT + filtered tool definitions + TOOL_CALLING_INSTRUCTIONS.

#### Bootstrap Prompt Addon (cycles 1-5 only)
...
#### Orchestration Guidance (only when Airflow is available)
...
#### Situational Awareness Guidance (SA_GUIDANCE)
...
#### Entity Guidance (ENTITY_GUIDANCE)
...
### Tool Calling Instructions (TOOL_CALLING_INSTRUCTIONS)

Appended after tool definitions in every system message that includes tools.
...
### Goal Context Template (GOAL_CONTEXT_TEMPLATE)

Injected in both PLAN and REASON user messages:
...
### Memory Context Template (MEMORY_CONTEXT_TEMPLATE)
...
**Routing**: Tier 3 dynamic (scored against CURATE; fixed weight 0.4, cooldown halves if previous was SURVEY)
**Purpose**: Analytical desk work -- judgment, not collection. Automated ingestion handles data; the agent reviews, connects, and acts on it.

### System Message

```
{SYSTEM_PROMPT}  (with cycle_number, context_tokens="survey")

{format_tool_definitions(SURVEY_TOOLS)}
...
**Routing**: Tier 2 modulo (every 9 cycles) or Tier 3 dynamic (scored by backlog, cap 0.6)
**Purpose**: Turn raw signals into curated events. Editorial judgment the automated system cannot provide.

### System Message

```
{SYSTEM_PROMPT}  (with cycle_number, context_tokens="curate")

{format_tool_definitions(CURATE_TOOLS)}
...
**Routing**: Tier 2 modulo (every 4 cycles)
**Purpose**: Find patterns, anomalies, and insights in accumulated data. No data ingestion.

### System Message

```
{SYSTEM_PROMPT}  (with cycle_number, context_tokens="analysis")

{format_tool_definitions(ANALYSIS_TOOLS)}
...
```
You are running a **dedicated analysis cycle**.
...
### Available Tools (ANALYSIS_TOOLS)
...
**Routing**: Tier 2 modulo (every 7 cycles)
**Purpose**: Fill gaps in the knowledge base -- entity enrichment, profile completion, data conflict resolution. Has external access (http_request).

### System Message

```
{SYSTEM_PROMPT}  (with cycle_number, context_tokens="research")

{format_tool_definitions(RESEARCH_TOOLS)}
...
### Available Tools (RESEARCH_TOOLS)
...
**Routing**: Tier 1 fixed (every 30 cycles)
**Purpose**: Self-improvement -- audit prompts, tools, operational effectiveness. Not introspection (knowledge base audit) and not research (entity enrichment).

### System Message

```
{SYSTEM_PROMPT}  (with cycle_number, context_tokens="evolve")

{format_tool_definitions(EVOLVE_TOOLS)}
...
**Routing**: Tier 1 fixed (every 10 cycles)
**Purpose**: Deep-dive investigation into a single situation or emerging pattern. Produces named Situation Briefs with competing hypotheses and falsifiable predictions.

### System Message

```
{SYSTEM_PROMPT}  (with cycle_number, context_tokens="synthesize")

{format_tool_definitions(SYNTHESIZE_TOOLS)}
...
```
You are running a **SYNTHESIZE cycle** — a deep-dive investigation into a single situation or emerging pattern.
...
### Available Tools (SYNTHESIZE_TOOLS)
...
## Cycle Type: INTROSPECTION

**Routing**: Tier 1 fixed (every 15 cycles)
**Purpose**: Deep self-assessment of the knowledge base. Graph audit, cross-domain pattern analysis, entity completeness, goal health, data quality, self-review, synthesis.

### System Message

```
{SYSTEM_PROMPT}  (with cycle_number, context_tokens="introspection")

{format_tool_definitions(INTROSPECTION_TOOLS)}
...
The INTROSPECTION cycle uses the `MISSION_REVIEW_PROMPT` as its user message (the same template serves both the introspection REASON+ACT phase and the legacy single-shot review):
...
If operator inbox messages are present, they are prepended to the user message.

### Available Tools (INTROSPECTION_TOOLS)

Defined in `phases/introspect.py`:
...
These prompts are used in specific phases or sub-calls, not as primary cycle prompts.

### PLAN Phase Prompt (PLAN_PROMPT)

Used in the PLAN phase of SURVEY (general) cycles. The system message for PLAN is SYSTEM_PROMPT + tool summary (compact name+description list). The user message includes goal context, memories, graph inventory, inbox, journal, reflection forward, then this prompt last.
...
### REASON Phase — Cycle Request (CYCLE_REQUEST)

Injected as the final section of the REASON user message, after the context data separator:
...
### Reporting Reminder (REPORTING_REMINDER)

Injected into the cycle request every `report_interval` cycles (default 5):
...
### REFLECT Phase Prompt (REFLECT_PROMPT)

**System message** (separate from SYSTEM_PROMPT -- a compact version):
...
You are Legba — a persistent autonomous intelligence analyst running a continuous cognitive loop. You observe, connect, and illuminate.
...
### NARRATE Phase Prompt (NARRATE_PROMPT — journal entry)
...
### Journal Consolidation Prompt (JOURNAL_CONSOLIDATION_PROMPT)

Used during INTROSPECTION cycles to consolidate recent journal entries.
...
### Analysis Report Prompt (ANALYSIS_REPORT_PROMPT)

Used during INTROSPECTION to produce the Current World Assessment. This is a separate LLM call with its own system/user messages.
...
You are Legba — autonomous intelligence analyst. Produce a differential Current World Assessment based on everything you know. Write for a decision-maker who needs to understand what has CHANGED since the last report. Lead with changes, not repetition. Be specific, cite entities and relationships from your knowledge. Use markdown formatting. Follow the numbered section structure exactly.
```
...
### Liveness Prompt (LIVENESS_PROMPT)

Used in the PERSIST phase to prove the LLM was in the loop by transforming a nonce.
...
### Re-grounding Prompt (REGROUND_PROMPT)

Injected periodically during long tool chains to prevent identity drift:
...
### Format Retry Prompt (FORMAT_RETRY_PROMPT)

Injected when the LLM response looks like it intended a tool call but the JSON did not parse:
...
### Budget Exhausted Prompt (BUDGET_EXHAUSTED_PROMPT)

Injected when the step budget is exhausted:
...
### Step Context Template (STEP_CONTEXT_TEMPLATE)

Used by the client for each step in the tool loop (single-turn pattern -- rebuilds full user message each step):
...
### Mission Review Prompt — Simple (MISSION_REVIEW_PROMPT_SIMPLE)

Legacy single-shot review prompt kept as fallback:

```
You are conducting a periodic strategic review of your goal tree.
...
### Source Discovery Prompt (SOURCE_DISCOVERY_PROMPT)

Used when the ingestion service is active and ACQUIRE becomes source discovery instead:

```
You are running a **source discovery cycle**.
...
**Source Discovery Tools (SOURCE_DISCOVERY_TOOLS)**:
...
### Acquire Prompt (ACQUIRE_PROMPT)

Used when the ingestion service is NOT active and the agent must fetch data itself:

```
You are running a **dedicated data acquisition cycle**.
...
### How System Messages Are Built

For **PLAN** and **REASON** (general SURVEY) phases, the system message is assembled by `_build_system_text()`:

```
SYSTEM_PROMPT (with {cycle_number}, {context_tokens})
+ BOOTSTRAP_PROMPT_ADDON (if cycle <= 5)
+ MEMORY_MANAGEMENT_GUIDANCE
+ EFFICIENCY_GUIDANCE
+ ANALYTICS_GUIDANCE
+ ORCHESTRATION_GUIDANCE (if Airflow available)
+ SA_GUIDANCE
+ ENTITY_GUIDANCE
+ format_tool_definitions(tool_data) (if REASON phase)
+ TOOL_CALLING_INSTRUCTIONS (if REASON phase)
```

For **specialized cycle types** (INTROSPECTION, RESEARCH, ANALYSIS, CURATE, EVOLVE, SYNTHESIZE, SURVEY), the system message is simpler:

```
SYSTEM_PROMPT (with {cycle_number}, {context_tokens}=cycle_type_name)
+ format_tool_definitions(filtered_tool_data)
+ TOOL_CALLING_INSTRUCTIONS
```

### How User Messages Are Built

For **PLAN** phase:
```
[World Briefing (if cycle <= 5)]
...
For **REASON** phase (general SURVEY):
```
--- CONTEXT DATA ---
[World Briefing (if cycle <= 5)]
GOAL_CONTEXT_TEMPLATE
...
For **specialized cycle types**, the user message is the cycle-specific prompt with `{placeholders}` filled, optionally prepended with inbox messages.

### Message Format

All LLM calls use a 2-message pattern: `[system, user]`. The system message contains identity, rules, and tool definitions. The user message contains context data and the task instruction.

Tool definitions are formatted as JSON blocks with parameter schemas. When `planned_tools` is specified in REASON phase, only those tools get full parameter details; others are listed as name + description with `explain_tool` available for on-demand lookup.
