...
### 24. `nlp_client` boot-singleton cannot re-resolve
...
| **Why deferred** | A live re-resolve would need the boot-time client factories to become reconcile-loop-driven (rebuild on a stack-component change event) rather than constructed once. That is a runtime-architecture change, out of current wave scope; the cheap, honest mitigation is the documented bring-up ORDER + a fail-loud boot signpost. |
