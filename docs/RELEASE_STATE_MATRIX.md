...
_Generated by `scripts/gen_release_state_matrix.py` from `legba-ui-v3/src/panel-registry/registry.ts` — do not hand-edit between the markers; re-run the script instead. 67 panel kinds registered — 64 live, 3 preview, 16 hidden (a panel can be both live/preview AND hidden — hidden is a navigation-tier flag, not a build state)._
...
| `system.narratives` | `system_narratives` | system | — | Narratives | no | personal, cis | **live** | no |
...
| `system.situations` | `system_situations` | system | — | Situation Trajectory | no | personal, cis | **live** | no |
