...
### 52. Receipt prompt columns — `prompt_rendered` is DECLINED (not unbound); `prompt_module_hash` IS unbound; the GEPA training input is the casualty

| | |
|---|---|
| **What** | `analyst_traces.prompt_module_hash` and `analyst_traces.prompt_rendered` are **0-populated over 187,550 rows, all-time** (live-verified 2026-08-04). A silent-absence census read that as two unbound columns. Only one of them is. |
| **`prompt_rendered` — declined by design, working substitute** | NOT a wiring gap. `src/legba/data/run_accounting.py` declines it explicitly, and `tests/data_pkg/test_run_accounting_receipt.py::test_prompt_is_evidenced_by_digest_not_by_its_text` pins the NULL: persisting the rendered prompt would put up to the full 32k-token input budget on every trace. The evidence it would carry is carried instead by a **bounded digest** — `llm_calls[].prompt_sha256` (64 hex) + `prompt_chars` — which is populated on **2,580 of 2,580** LLM-bearing traces (100%), and which lets an auditor re-render and prove identity without the blob. **Neither bound nor retired, on cost:** retiring is the EXPENSIVE option, because the key sits inside `compute_receipt_hash`'s canonical payload (`provenance/_core.py`). Dropping it means either changing that payload — invalidating re-verification of all 187,550 existing receipts and breaking the tamper-evident chain — or pinning the key to `None` while dropping the column, which is strictly more code than leaving a nullable `text` column that costs 0 bytes. Left as-is, documented here so the next census stops counting a decision as a defect. |
...
| **The live casualty** | `optimizer.py` selects `t.prompt_rendered AS input` for the GEPA training set, so **every training row's input is `""`** and has been all along — GEPA optimizes against empty inputs and said nothing. Made audible 2026-08-04 (`optimizer.training_set.all_inputs_empty` warning when a whole set is empty); the actual fix is to source the training input from somewhere other than a deliberately-NULL column, which is part of the same scoped item. |
| **Guard rail** | Nothing fabricated: a NULL column reads as NULL, `compute_receipt_hash` hashes the `None` honestly, and `lineage_api` re-feeds the stored value so historical receipts still verify byte-for-byte. The empty GEPA input is now loud rather than silent, and GEPA promotion is operator-gated regardless. (No allowlist line — no stub symbol; two nullable columns and one declined field.) |
