...
class WorldBaselineSeedSource:
    ...
    def map(self, raw: dict[str, Any]) -> Iterable[SeedPayload]:
        ...
        # 3) Active conflicts → typed SIGNED -1 nexuses (the curated
        #    current-world-state layer). Each conflict lists its belligerents;
        #    we emit one signed edge per ORDERED pair of distinct belligerents
        #    (A→B and B→A) so the grounding resolver — which matches on
        #    nexuses.subject — surfaces the war whichever side a country analyst
        #    is scoped to. Distinct (subject, object) triples never supersede one
        #    another, so a country in several wars keeps one open edge per
        #    opponent. Adding a war is a one-line YAML add under ``conflicts``.
        for conflict_row in conflicts:
            ...
            for b in belligerents:
                if b not in countries:
                    yield SeedEntity(canonical_name=b, entity_class="country")
                    countries.add(b)
            for subject in belligerents:
                for obj in belligerents:
                    if subject == obj:
                        continue
                    yield SeedNexus(
                        subject=subject,
