...
| **Why deferred** | **L-114 (the embedder-through-port half) is now DONE**: the host threads the hosted embedding client into `SubstrateGroundingResolver(pg_pool=…, embedder=embedding_service)` (via `analyst_deps_builder._build_grounding_hook`), so the resolver already carries the embedder — it just does not consume it yet. Two blockers remain for the actual Tier-2 read: (a) a curated `world_context` Qdrant collection to embed into (un-provisioned infra), and (b) a `resolve_world_context` semantic read method on the resolver + a source branch in the hook. The structured substrate already covers the load-bearing case (current officeholders + bloc memberships) without an embedding dependency, so the semantic brief remains a deliberate follow-up rather than a blocker on the live grounding path. |
