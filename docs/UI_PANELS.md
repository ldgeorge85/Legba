...
| **Cycles** | Paginated table of agent cycle history showing cycle number, type (with color-coded badge), duration, tool call count, LLM call count, events produced, and error count. 50 per page. | `GET /api/v2/cycles` | Pagination. Rows color-coded by cycle type (EVOLVE, INTROSPECTION, ANALYSIS, RESEARCH, CURATE, SURVEY, SYNTHESIZE). |
