...
| Contested-claims arbiter ("alternate facts") | **built** | Detect-only, flag-gated (#101). A deterministic `Q·C·R·F` arbiter (per-fact `source_credibility`, migration 0054) keeps disputed `(subject, predicate)` values coexisting open and surfaces a credibility-weighted winner in a sidecar (migration 0055) — it **never mutates a fact** and abstains on a near-tie. The write-path coexistence (`LEGBA_FACT_CONTENTION`) and the optional vLLM near-tie tie-break (`LEGBA_FACT_CONTENTION_LLM_TIEBREAK`) both ship **OFF by default**, enabled only on this instance. The deterministic arbiter, coexistence, and read API (`GET /api/v1/contention`) are proven live; a successful LLM tie-break *pick* is not yet observed live. See [docs/ANALYSIS.md](docs/ANALYSIS.md) §7.11. |
